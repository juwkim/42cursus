...
void number(long num, int base, int field_width, int precision, int flags, int *printed)
{
	...
	cond = 1;
	if (precision == -1 && (flags & ZEROPAD))
		cond = 0;

	iszero = (num == 0);
	isprezero = precision == 0;

	if (iszero && isprezero && field_width != -1)
		*printed += write(1, " ", 1);
	...
	if ((flags & ADDRESS) || (!iszero && (flags & SPECIAL))) field_width -= 2;
	...
	if ((flags & ADDRESS) || (!iszero && (flags & SPECIAL))) {
		if (locase)
